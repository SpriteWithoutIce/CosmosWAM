...
def _obs_to_model_input(
    ...
    if num_cameras == 1:
        ...
        elif concatenation is None or concatenation == "none":
...
def eval_single_process(cfg: DictConfig):
    ...
    if missing:
        logging.warning("Missing checkpoint keys, first 20: %s", missing[:20])
    if unexpected:
        logging.warning("Unexpected checkpoint keys, first 20: %s", unexpected[:20])
